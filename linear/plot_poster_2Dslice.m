...
set(gca,'XTick',[])
ylabel('Topography','fontsize',15)

subplot('position',[0.1 0.1 0.8 0.7])
contourf(xaxis,depth_prof,vel_prof,100);
shading flat
colormap(seiscmap)
colorbar('south');
xlim([min(xaxis) max(xaxis)])
text(min(xaxis),-3.5,'SW','fontsize',20)
text(max(xaxis)-0.05,-3.5,'NE','fontsize',20)
axis ij
caxis([4.0 4.7])
ylim([20 120])
xlabel('Longtitude','fontsize',20);
ylabel('Depth','fontsize',15);
filename = ['pics/shear2DsliceEW.png'];
export_fig(filename,'-transparent','-png','-m2')
